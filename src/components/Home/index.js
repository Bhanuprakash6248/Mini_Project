...
class Home extends Component {
  ...
  onEmojiNameChange = e => {
    this.setState({activeEmojiName: e.target.value})
  }

  onDayChange = e => {
    this.setState({activeDay: e.target.value})
  }

  onChangeMonth = month => {
    this.setState({month}, this.getNameDayCount)
  }

  getNameDayCount = () => {
    const {activeDay, activeEmojiName, calenderList, month} = this.state
    ...
    this.setState({nameDayCount: `0${count}`})
  }

  onDateOfCalender = (id, month) => {
    const {calenderList, activeEmojiId} = this.state

    const datesList = calenderList[month - 1].dates

    const liObj = datesList.find(each => each.id === id)

    const activeEmojiObj = emojisList.find(each => each.id === activeEmojiId)
    console.log(activeEmojiObj)
    let newObj
    ...
    if (
      ...
      newObj = {
        ...liObj,
        emojiUrl: activeEmojiObj.emojiUrl,
        emojiName: activeEmojiObj.emojiName,
      }
    } else if (liObj.emojiName === activeEmojiObj.emojiName) {
      newObj = {...liObj, emojiName: '', emojiUrl: ''}
    }

    const newDateList = datesList.map(each => {
      if (each.id === id) {
        return newObj
      }
      return each
    })
    const newMonth = {...calenderList[month - 1], dates: newDateList}
    calenderList[month - 1] = newMonth
    const newCalenderList = calenderList

    this.setState({calenderList: newCalenderList}, this.getNameDayCount)
  }
        ...
        <div className="home-bg">
          ...
          <div className="emoji-container">
            <ul className="emoji-ul-container">
              {emojisList.map(each => {
                const selectEmoji = () => {
                  this.emojiClicked(each.id)
                  console.log(each.id)
                }
                const emojiClassName =
                  activeEmojiId === each.id ? 'active-emoji' : null
                const emojiButton =
                  activeEmojiId === each.id ? 'activeEmojiButton' : null
                const emojiText =
                  activeEmojiId === each.id ? 'activeEmojiPara' : null

                return (
                  <li key={each.id} className="emoji-listItem">
                    <button
                      className={`emoji-btn ${emojiButton}`}
                      type="button"
                      onClick={selectEmoji}
                    >
                      <p className={`emoji-name ${emojiText}`}>
                        {each.emojiName}
                      </p>
                      <img
                        src={each.emojiUrl}
                        alt={each.emojiName}
                        className={`emoji-img ${emojiClassName}`}
                      />
                    </button>
                  </li>
                )
          ...
          <div>
            <Calender
              ...
              onDateOfCalender={this.onDateOfCalender}
            />
          </div>
          <div className="dropDowns-con">
            <select
            ...
            >
              {emojisList.map(each => (
                <option key={each.id}>{each.emojiName}</option>
              ))}
            </select>
            <select
              className="select-con"
            ...
            >
              {daysList.map(each => (
                <option key={each.id}>{each.day}</option>
              ))}
            </select>
